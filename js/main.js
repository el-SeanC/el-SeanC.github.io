(function($) {
    ...
    * -------------------------------------------------- */
    ...
    /* adjust about section aesthetics on window size
    * -------------------------------------------------- */
    /*  
    function elimBr(windowSize) {
        if (windowSize.matches) { // If media query matches
            var aboutWithoutBr = "Professional web developer focused on creating customer-centric websites & applications for the end user. <br>"
            var aboutContent = document.getElementById("s-hero__content-about");
            aboutContent.innerHTML = aboutWithoutBr;
        }
     }

    var windowSize = window.matchMedia("(max-width: 900px)")
    elimBr(windowSize) // Call listener function at run time
    windowSize.addListener(elimBr) // Attach listener function on state changes
    */
    ...
    * ------------------------------------------------------ */
    const ssAOS = function() {
        ...
        AOS.init( {
            offset: 0, // Default Is 100, 0 Seems More Responsive.
            duration: 600,
            easing: 'ease-in-out',
            delay: 300,
            once: true,
